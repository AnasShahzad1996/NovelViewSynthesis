...
from kilonerf_simple import KiloNeRFSimple
...
class ModelSelection:
    ...
    def getModel(cls, config: Config, n_in, n_out, device, model_idx):
        ...
        if config.activation[i] == 'relu':
            ...
            return KiloNeRFSimple(config.layers[i], config.layerWidth[i], n_in=n_in, n_out=n_out, skips=[config.skips[i]], use_viewdirs=True, net_idx=i, config=config).to(device)
