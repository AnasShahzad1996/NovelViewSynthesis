...
class KiloNeRF(nn.Module):
    ...
    def extract_single_network(self, network_index):
        single_network = KiloNeRF(1, self.num_position_channels, self.num_direction_channels, self.num_output_channels,
...
def build_multi_network_from_single_networks(single_networks, transpose_weight = True, linear_implementation = 'multimatmul', view_dependent_dropout_probability=-1):
    ...
    multi_network = KiloNeRF(num_networks, p.num_position_channels, p.num_direction_channels, p.num_output_channels, p.hidden_layer_size, p.num_hidden_layers, p.refeed_position_index, p.late_feed_direction,
