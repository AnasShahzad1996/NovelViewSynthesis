...
def render_path(render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0,to_stack=False):
    ...
    for i, c2w in enumerate(tqdm(c2ws)):
        ...
        if calculate_quality_metrics:
            ...
            psnr = mse2psnr(mse)
            print ("these are the dimensions",rgb.shape,"|",gt_img_numpy.shape,(gt_img_numpy.max() - gt_img_numpy.min()))
            ssim = calculate_ssim(rgb.cpu().numpy(), gt_img_numpy, data_range=gt_img_numpy.max() - gt_img_numpy.min(), multichannel=True,channel_axis=2)
...
def train(cfg, log_path, render_cfg_path):
    ...
    print ("it loaded the data : ",cfg['dataset_type'])
    print (cfg)
    #print (1/0)
    # Load data
    ...
    if cfg['dataset_type'] == 'blender' or cfg['dataset_type'] == 'nsvf':
        if cfg['blender_white_background'] and images.shape[-1] == 4:
            ...
            #complez = (1.-images[...,-1:])
            #print ("you can give me medicine , ",complez.shape)
            print ("what is the dimension :", images[...,-1].shape,"|",images[...,-1:].shape)
            alpha = images[..., -1:].copy()
            # images[..., :3] * alpha +
            print ("llllllllllllll 1")
            images[...,0] = images[...,0] * np.squeeze(alpha)
            print ("llllllllllllll 2")
            images[...,1] = images[...,1] * np.squeeze(alpha)
            print ("llllllllllllll 3")
            images[...,2] = images[...,2] * np.squeeze(alpha)
            import gc
            gc.collect()
            temp = images[...,:3] + (1. - alpha)
            images = None
            images = temp
            gc.collect
            print ("veer zahra : ",images.shape)
            print ("tu mila jis tarah zuban mile")
        else:
            images = images[...,:3]
    print ("coverted alpha into white blah the blah the blahd")    
    ...
    print ("it came to the checkpoints blah the blah")
    # Checkpoint loading
    ...
    print ("it came to the optimizer blah the blah..")
    # Create optimizer
    cfg['initial_learning_rate'] = float(cfg['initial_learning_rate'])
    print ("render poses -1.....")
    ...
    if cfg['render_only']:
        ...
        model.eval()
        with torch.no_grad():
            print ("cccxxxxxxxxxxxxxxxxxx")
            if render_subset != 'custom_path':
                # load images for subset to be rendered (train/val/test)
                print ("wafsdfa df asdf asdf asdf ",i_render,images.shape)
            ...
            rgbs, _ = render_path(render_poses, intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=images,
                savedir=testsavedir, render_factor=cfg['render_factor'],to_stack=True)
            import gc
            gc.collect()
            ...
            np.save("resume_from_here.npy", rgbs)
            print ("did it come out")
            return
    print ("render poses 2.....")
...
def main():
    ...
    print ("it read the configs . blah the blah")
    restarting_job = train(cfg, log_path, render_cfg_path)
    print ("it came out finaly")
